...
import com.momo.merchants.gui.ModGuiElementLoader;
import com.momo.merchants.entity.projectiles.EntityIdlProjectile;
import com.momo.merchants.entity.projectiles.ProjectileArgs;
import com.momo.merchants.util.NBTStrDef.IDLNBTUtil;
...
//    }
    ...
    void TestEnchant(World worldIn, EntityPlayer playerIn, EnumHand handIn)
    {
        ...
        for (Enchantment enchantment : Enchantment.REGISTRY)
        {
            ...
            MoMoFramework.Log("[%s]Lv %d to %d, canApplyAtEnchantingTable = %s, judgement[%s,%s,%s]",
                    enchantment.getName(),
            ...
            if (judge1 && judge2 || judge3)
            {
                ...
                {
                    MoMoFramework.Log("[%s] Lv.%d available range = %d to %d", enchantment.getName(), i,
                    ...
                    {
                        MoMoFramework.Log("[%s] successfully elected", enchantment.getName());
                        break;
                    }
                }
            }
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        ...
        if (index == 2 && !worldIn.isRemote) {
            ...
            MoMoFramework.Log("bullet pos = %s", bullet.getPosition());
        ...
        if (!worldIn.isRemote)
        {
            ...
            if (index == 3)
            {
                ...
                playerIn.openGui(MoMoFramework.instance, ModGuiElementLoader.GUI_RESEARCH, worldIn, pos.getX(), pos.getY(), pos.getZ());
            }
            //else

            //todo: optimize
            //playerIn.openGui(MoMoFramework.instance, ModGuiElementLoader.GUI_DEMO, worldIn, pos.getX(), pos.getY(), pos.getZ());
...
//            }
        ...
        //MoMoFramework.Log("NBT len = %d", IDLNBTUtil.GetString(stack, "TEST", "").length());
