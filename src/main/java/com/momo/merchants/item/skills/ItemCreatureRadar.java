...
import com.momo.merchants.util.IDLGeneral;
...
import static com.momo.merchants.util.NBTStrDef.IDLNBTDef.STATE;
        ...
        if (!world.isRemote) {
            ...
            if (!cloneResult.isEmpty())
            {
                ...
                if (name != null && EntityList.ENTITY_EGGS.containsKey(name))
                {
                    ...
                    MoMoFramework.LogWarning(getNBT(stack).toString());
...
//        Entity entity = EntityList.createEntityByIDFromName(resourcelocation, worldIn);
    ...
    public void onUpdate(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        ...
        if (!worldIn.isRemote)
        {
            ...
            MoMoFramework.Log("update:",getNBT(stack).toString());
            ...
            {
                //MoMoFramework.Log(String.format("[Active]Nearby %s -> %s" , entity.getName() ,entity.getAttackTarget()));
                if (entity.getClass() == s && entity != entityIn)
                {
                    detection++;
                    //MoMoFramework.Log("[Active]Detected!");
                }
            }
            ...
            {
                MoMoFramework.LogWarning("Changed to " + detection);
                SetInt(stack, STATE, detection);
